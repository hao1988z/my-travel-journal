...
function bindEvents() {
  ...
  $("lightboxCoverBtn").addEventListener("click", setLightboxPhotoAsCover);
...
function renderDrawer(trip, sharedMode) {
  ...
  const photoGallery = photos.length
    ? `<div class="photo-gallery">${photos.map((photo, index) => `
        <figure class="photo-tile">
          ...
          ` : ""}
          ${!sharedMode ? `<button class="photo-delete-btn" type="button" data-photo-id="${escapeHtml(photo.id || "")}" data-photo-path="${escapeHtml(photo.storage_path || "")}">刪除</button>` : ""}
        </figure>
      `).join("")}</div>`
    : "";
    ...
    ? `
      <div class="photo-download-panel">
        ...
        <div class="photo-download-actions">
         ...
         <button class="btn btn-primary" type="button" id="downloadAllPhotosBtn">下載全部（${photos.length}）</button>
          ${!sharedMode ? `<button class="btn btn-danger" type="button" id="deleteSelectedPhotosBtn" disabled>刪除所選</button>` : ""}
       </div>
      </div>
    `
    : "";
 ...
 $("downloadAllPhotosBtn")?.addEventListener("click", (event) => {
   downloadPhotos(photos, trip, event.currentTarget);
 });
  $("deleteSelectedPhotosBtn")?.addEventListener("click", (event) => {
    const selected = photos.filter((photo, index) => state.downloadSelection.has(getPhotoSelectionKey(photo, index)));
    deleteSelectedPhotos(selected, trip, event.currentTarget);
  });
 $("drawerBody").querySelectorAll(".photo-delete-btn").forEach((button) => {
    button.addEventListener("click", () => deletePhoto(button.dataset.photoId, trip.id, button.dataset.photoPath));
  });
}
...
function openTripDialog(trip = null) {
  ...
  if (trip) {
    ...
    if (existingCount) {
      $("photoSelectionSummary").textContent = `已上傳 ${existingCount} 張；可在旅程詳情中單張或多張刪除。`;
...
async function deleteTripPhotoRow(photo, trip) {
...
async function deletePhoto(photoId, tripId, storagePath = "") {
  const trip = state.trips.find((item) => item.id === tripId);
  const photo = getTripPhotos(trip).find((item) => String(item.id || "") === String(photoId || "")
    || (!item.id && item.storage_path === storagePath));
  if (!photo || !confirm("確定要刪除這張照片嗎？")) return;

  try {
    await deleteTripPhotoRow(photo, trip);
  } catch (error) {
    console.error("[deletePhoto]", error, photo);
    return toast(`照片刪除失敗：${error.message || "請稍後再試"}`);
  }
...
async function deleteSelectedPhotos(photos, trip, sourceButton) {
  if (!photos.length) return toast("請先勾選要刪除的照片");
  if (!confirm(`確定要刪除選取的 ${photos.length} 張照片嗎？此動作無法復原。`)) return;

  const originalText = sourceButton?.textContent || "刪除所選";
  const deletedPhotos = [];
  if (sourceButton) sourceButton.disabled = true;
  try {
    for (let index = 0; index < photos.length; index += 1) {
      await deleteTripPhotoRow(photos[index], trip);
      deletedPhotos.push(photos[index]);
      if (sourceButton) sourceButton.textContent = `刪除中 ${deletedPhotos.length}/${photos.length}`;
    }
    ...
    if (paths.length) {
      ...
      if (storageError) {
        console.error("[deleteSelectedPhotos.storage]", storageError, paths);
        storageClean = false;
      }
    }
    ...
    await loadTrips();
    openTrip(trip.id);
    toast(storageClean ? `已刪除 ${deletedPhotos.length} 張照片` : `已刪除 ${deletedPhotos.length} 筆紀錄，但部分雲端檔案清理失敗`);
  } catch (error) {
    console.error("[deleteSelectedPhotos]", error);
...
function getCoverUrl(trip) {
  ...
  return getPhotoUrl(photos.find((photo) => photo.storage_path === trip.cover_path)) || getPhotoUrl(photos[0]);
}
...
function updateDownloadSelection(photos) {
 ...
 const button = $("downloadSelectedPhotosBtn");
  const deleteButton = $("deleteSelectedPhotosBtn");
 if (status) status.textContent = count ? `已選擇 ${count} 張` : "尚未選擇照片";
  ...
  if (deleteButton) {
    ...
    deleteButton.textContent = count ? `刪除所選（${count}）` : "刪除所選";
  }
}
...
function renderPhotoLightbox() {
  ...
  const coverButton = $("lightboxCoverBtn");
  const canSetCover = !state.lightboxSharedMode && !!state.user && !!photo.storage_path;
  if (coverButton) {
    coverButton.hidden = !canSetCover;
    coverButton.disabled = !canSetCover || photo.storage_path === state.lightboxTrip.cover_path;
    coverButton.textContent = photo.storage_path === state.lightboxTrip.cover_path ? "目前封面" : "設為封面";
  }
...
function openCoverPicker(trip) {
  const photos = getTripPhotos(trip).filter((photo) => getPhotoUrl(photo));
  if (!photos.length) return toast("這趟旅程目前沒有可預覽的照片");
  state.editingTrip = trip;
  $("coverPickerStatus").textContent = `共 ${photos.length} 張照片 · 點選照片即可設為封面`;
  renderCoverPicker(trip, photos);
  if (!$('coverPicker').open) $('coverPicker').showModal();
}

function closeCoverPicker() {
  if ($("coverPicker").open) $("coverPicker").close();
}
...
function renderCoverPicker(trip, photos) {
  ...
  grid.innerHTML = photos.map((photo, index) => {
    ...
    return `<button class="cover-picker-card${isCurrent ? " is-current" : ""}" type="button" data-cover-photo-index="${index}" aria-pressed="${isCurrent ? "true" : "false"}" aria-label="${escapeHtml(isCurrent ? `${label}，目前封面` : `將 ${label} 設為封面`)}">
      <span class="cover-picker-image"><img src="${escapeHtml(getPhotoUrl(photo))}" alt="${escapeHtml(label)}" loading="lazy" decoding="async"></span>
...
function handleCoverPickerClick(event) {
  ...
  if (!card) return;
  const photos = getTripPhotos(state.editingTrip).filter((photo) => getPhotoUrl(photo));
  const photo = photos[Number(card.dataset.coverPhotoIndex)];
  if (photo) setTripPhotoAsCover(photo, state.editingTrip);
}

async function setLightboxPhotoAsCover() {
  const photo = state.lightboxPhotos[state.lightboxIndex];
  if (photo) await setTripPhotoAsCover(photo, state.lightboxTrip);
}

async function setTripPhotoAsCover(photo, trip) {
  if (state.sharedMode || !state.user || !trip || !photo?.storage_path) return;
  if (photo.storage_path === trip.cover_path) return;

  const buttons = [$("lightboxCoverBtn"), ...document.querySelectorAll("[data-cover-photo-index]")].filter(Boolean);
  buttons.forEach((button) => {
    button.disabled = true;
    if (button.id === "lightboxCoverBtn") button.textContent = "儲存中…";
  });
  ...
  try {
    const { data, error } = await client
      ...
      .single();
    if (error) throw error;
    if (!data?.id || data.cover_path !== photo.storage_path) throw new Error("封面設定沒有成功保存");
    ...
    renderCoverPicker(trip, getTripPhotos(trip).filter((item) => getPhotoUrl(item)));
    $("coverPickerStatus")?.replaceChildren(document.createTextNode("已設定目前封面 · 點選其他照片可更換"));
    renderPhotoLightbox();
    toast("已設定為旅程封面");
  } catch (error) {
    console.error("[setTripPhotoAsCover]", error);
    const message = error?.code === "PGRST204" || error?.code === "42703"
      ? "請先在 Supabase 執行 20260824_cover_path.sql"
      : error.message || "請稍後再試";
    $("coverPickerStatus")?.replaceChildren(document.createTextNode(`設定失敗：${message}`));
    toast(`設定封面失敗：${message}`);
    renderPhotoLightbox();
  }
}
